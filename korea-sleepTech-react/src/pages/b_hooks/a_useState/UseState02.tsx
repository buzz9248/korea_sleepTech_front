...
function UseState02() {
  ...
  const handleLoginChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setLogin({
      ...
      // 변경하고자 하는 name 키를 가진 value 값을 변경 (해당 필드의 값 업데이트)
      [name]: value,
    })
  };
  ...
  return (
    <div>
      ...
      <form onSubmit={handleLoginSubmit}>
        <input type="text" name="id" placeholder="아이디" value={login.id} onChange={handleLoginChange} />
        <input type="text" name="password" placeholder="비밀번호" value={login.password} onChange={handleLoginChange} />
