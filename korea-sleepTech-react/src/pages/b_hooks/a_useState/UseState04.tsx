...
function UseState04() {
  ...
  const onSignUpSubmit = (e: React.FormEvent) => {
    ...
    if (!id) { // id 입력 필드가 비워진 경우 (빈 문자열 - false)
  ...
  return (
    ...
    >
      ...
      <form onSubmit={onSignUpSubmit}>
        <div>
          <label>아이디:
            <input type="text" name='id' value={formData.id} onChange={onSignUpInputChange} />
          </label>
          {errors.id && (<p style={{ color: 'red' }}>{errors.id}</p>)}
        </div>
        <div>
          <label>비밀번호:
            <input type="text" name='password' value={formData.password} onChange={onSignUpInputChange} />
          </label>
          {errors.password && (<p style={{ color: 'red' }}>{errors.password}</p>)}
        </div>
        <div>
          <label>이메일:
            <input type="text" name='email' value={formData.email} onChange={onSignUpInputChange} />
          </label>
