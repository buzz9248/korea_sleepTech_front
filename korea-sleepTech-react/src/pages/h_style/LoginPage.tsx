...
// - 해당 주석이 없으면 css={s.layout} 같은 방식이 작동 X
...
function LoginPage() {
  ...
  const [inputRefs] = useState([useRef<HTMLInputElement>(null), useRef<HTMLInputElement>(null)]);
  ...
  const handleInputOnKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      ...
      // Enter 키가 눌러진 이벤트 객체의 현재 DOM 요소와 일치하는 경우의 index 번호를 반환
  ...
  return (
    <div css={s.layout}>
      <div css={s.main}>
        ...
        <button onClick={handleLoginSubmitOnClick} ref={buttonRefs[0]}>로그인</button>
      </div>
